...
/**
 * This task handles the processing of {@link Chunk}s to unload {@link SkyHopper}s in those chunks.
 */
public class SkyHopperUnloadTask  extends BukkitRunnable {
    ...
    /**
     * Process the chunk unload queue and unload any SkyHoppers in the chunk.
     * Limited to {@link #chunksPerRun}.
     */
    @Override
    public void run() {
        int chunksProcessed = 0;

        @Nullable Chunk chunk = skyHopperProcessor.getNextQueuedUnloadChunk();
        while(chunk != null && chunksProcessed <= chunksPerRun) {
            skyHopperProcessor.unLoadSkyHoppersInChunk(chunk);

            chunksProcessed++;

            chunk = skyHopperProcessor.getNextQueuedUnloadChunk();
        }
    }
}
