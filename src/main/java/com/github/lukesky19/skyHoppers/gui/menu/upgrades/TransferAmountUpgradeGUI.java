...
public class TransferAmountUpgradeGUI extends SkyHopperGUI {
    ...
    public boolean create() {
        if(guiConfig == null) {
            logger.warn(AdventureUtil.serialize("Unable to create the InventoryView for the transfer amount upgrade GUI due to invalid GUI configuration."));
            return false;
        }

        GUIType guiType = guiConfig.guiType();
        if(guiType == null) {
            logger.warn(AdventureUtil.serialize("Unable to create the InventoryView for the transfer amount upgrade GUI due to an invalid GUIType"));
            return false;
        }

        String guiName = guiConfig.name();
        if(guiName == null) {
            logger.warn(AdventureUtil.serialize("Unable to create the InventoryView for the transfer amount upgrade GUI due to an invalid gui name."));
            return false;
        }
    ...
    public boolean update() {
        if(guiConfig == null) {
            logger.warn(AdventureUtil.serialize("Unable to decorate the GUI due to invalid configuration for the transfer amount upgrade GUI."));
            if(isOpen) close();
            return false;
        }

        if(inventoryView == null) {
            logger.warn(AdventureUtil.serialize("Unable to update the transfer amount upgrade GUI as the InventoryView was not created."));
            if(isOpen) close();
            return false;
        }
        ...
        if(settings == null) {
            logger.warn(AdventureUtil.serialize("Unable to update the transfer amount upgrade GUI as the plugin settings are invalid."));
            if(isOpen) close();
            return false;
        }
        ...
        if(upgrades == null) {
            logger.warn(AdventureUtil.serialize("Unable to update the transfer amount upgrade GUI as the transfer amount upgrade settings are invalid."));
            if(isOpen) close();
            return false;
        }
    ...
    private void createExitButton() {
        ...
        if(buttonConfig.slot() == null) {
            logger.warn(AdventureUtil.serialize("Unable to create the exit button in the transfer amount upgrade gui due to no slot configured."));
            return;
        }
    ...
    private void createIncreaseButton(@NotNull TreeMap<Integer, Double> upgrades) {
        ...
        if(skyHopper.getTransferAmount() != skyHopper.getMaxTransferAmount()) {
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.serialize("Unable to create the increase button in the transfer amount upgrade gui due to no slot configured."));
                return;
            }
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.serialize("Unable to create the increase max button in the transfer amount upgrade gui due to no slot configured."));
                return;
            }
    ...
    private void createDecreaseButton(@NotNull TreeMap<Integer, Double> upgrades) {
        ...
        if(skyHopper.getTransferAmount() != upgrades.firstKey()) {
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.serialize("Unable to create the decrease button in the transfer amount upgrade gui due to no slot configured."));
                return;
            }
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.serialize("Unable to create the decrease min button in the transfer amount upgrade gui due to no slot configured."));
                return;
            }
    ...
    private void createUpgradeButton(@NotNull TreeMap<Integer, Double> upgrades) {
        ...
        if(nextUpgrade.isPresent()) {
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.serialize("Unable to create the upgrade button in the transfer amount upgrade gui due to no slot configured."));
                return;
            }
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.serialize("Unable to create the upgrade max button in the transfer amount upgrade gui due to no slot configured."));
                return;
            }
    ...
    private void createDummyButtons() {
        ...
        guiConfig.entries().dummyButtons().forEach(buttonConfig -> {
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.serialize("Unable to add a dummy button to the transfer amount upgrade GUI due to an invalid slot."));
                return;
            }
