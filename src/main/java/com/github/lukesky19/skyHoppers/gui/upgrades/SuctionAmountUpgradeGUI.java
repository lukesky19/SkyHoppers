 ...
 */
public class SuctionAmountUpgradeGUI extends ChestGUI {
    ...
    public SuctionAmountUpgradeGUI(
        ...
        guiConfig = guiManager.getUpgradeConfig("suction_amount.yml");
        if(guiConfig == null) {
            throw new RuntimeException("Unable to find loaded config file suction_amount.yml.");
        }

        guiType = GUIType.getType(guiConfig.guiType());
        if(guiType == null) {
            throw new RuntimeException("Unknown GUIType " + guiConfig.guiType() + " in suction_amount.yml");
        }

        if(guiConfig.name() == null) {
            throw new RuntimeException("GUI name is null in suction_amount.yml.");
        }
    ...
    public void update() {
        ...
        TreeMap<Integer, Double> displayUpgrades = settingsManager.getSuctionAmountUpgrades();
    ...
    private void createIncreaseButton(@NotNull SkyHopper skyHopper, @NotNull TreeMap<Integer, Double> displayUpgrades) {
        ...
        if(skyHopper.suctionAmount() != skyHopper.maxSuctionAmount()) {
            ...
            if (increaseConfig.item().material() != null) {
                ...
                if (material != null) {
                    ...
                    Optional<Map.Entry<Integer, Double>> displayUpgrade = getLargerAmount(displayUpgrades, skyHopper.suctionAmount());
                    if (displayUpgrade.isEmpty()) return;

                    List<TagResolver.Single> lorePlaceholders = List.of(
                            Placeholder.parsed("current", String.valueOf(skyHopper.suctionAmount())),
                    ...
                    increaseBuilder.setAction(inventoryClickEvent -> {
                        ...
                        Optional<Map.Entry<Integer, Double>> optionalUpgrade = getLargerAmount(upgrades, skyHopper.suctionAmount());
    ...
    private void createDecreaseButton(@NotNull SkyHopper skyHopper, @NotNull TreeMap<Integer, Double> displayUpgrades) {
        ...
        if(skyHopper.suctionAmount() != displayUpgrades.firstKey()) {
            ...
            if (decreaseConfig.item().material() != null) {
                ...
                if (material != null) {
                    ...
                    Optional<Map.Entry<Integer, Double>> displayUpgrade = getSmallerAmount(displayUpgrades, skyHopper.suctionAmount());
                    if (displayUpgrade.isEmpty()) return;
                    ...
                    decreaseBuilder.setAction(inventoryClickEvent -> {
                        ...
                        Optional<Map.Entry<Integer, Double>> optionalUpgrade = getSmallerAmount(upgrades, skyHopper.suctionAmount());
                        if (optionalUpgrade.isPresent()) {
                            Map.Entry<Integer, Double> upgrade = optionalUpgrade.get();
    ...
    private void createUpgradeButton(@NotNull SkyHopper skyHopper, @NotNull TreeMap<Integer, Double> displayUpgrades) {
        ...
        Optional<Map.Entry<Integer, Double>> displayUpgrade = getNextUpgrade(displayUpgrades, skyHopper.maxSuctionAmount());
        if(displayUpgrade.isPresent()) {
            ...
            if (upgradeConfig.item().material() != null) {
                ...
                if (material != null) {
                    ...
                    List<TagResolver.Single> lorePlaceholders = List.of(
                            Placeholder.parsed("current", String.valueOf(skyHopper.maxSuctionAmount())),
                    ...
                    upgradeBuilder.setAction(inventoryClickEvent -> {
                        ...
                        Optional<Map.Entry<Integer, Double>> optionalUpgrade = getNextUpgrade(upgrades, skyHopper.maxSuctionAmount());
                        ...
                        if (plugin.getEconomy().getBalance(player) >= upgradePrice) {
                            if (currentSkyHopper.location() != null && currentSkyHopper.location().getBlock().getState(false) instanceof Hopper hopper) {
                                ...
                                List<TagResolver.Single> placeholders = List.of(
                                        Placeholder.parsed("current", String.valueOf(currentSkyHopper.maxSuctionAmount())),
                                        Placeholder.parsed("next", String.valueOf(upgradeAmount)));

                                player.sendMessage(FormatUtil.format(player, locale.prefix() + locale.suctionAmountUpgrade(), placeholders));
