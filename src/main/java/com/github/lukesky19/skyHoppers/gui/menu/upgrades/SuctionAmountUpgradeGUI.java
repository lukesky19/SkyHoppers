...
/**
 * This class lets Players upgrade the amount of items a SkyHopper can suction at once.
 */
public class SuctionAmountUpgradeGUI extends SkyHopperGUI {
    ...
    public boolean create() {
        if(guiConfig == null) {
            logger.warn(AdventureUtil.deserialize("Unable to create the InventoryView for the suction amount upgrade GUI due to invalid GUI configuration."));
            return false;
        }

        GUIType guiType = guiConfig.guiType();
        if(guiType == null) {
            logger.warn(AdventureUtil.deserialize("Unable to create the InventoryView for the suction amount upgrade GUI due to an invalid GUIType"));
            return false;
        }

        String guiName = guiConfig.name();
        if(guiName == null) {
            logger.warn(AdventureUtil.deserialize("Unable to create the InventoryView for the suction amount upgrade GUI due to an invalid gui name."));
            return false;
        }
    ...
    public boolean update() {
        if(guiConfig == null) {
            logger.warn(AdventureUtil.deserialize("Unable to decorate the GUI due to invalid configuration for the suction amount upgrade GUI."));
            if(isOpen) close();
            return false;
        }

        if(inventoryView == null) {
            logger.warn(AdventureUtil.deserialize("Unable to update the suction amount upgrade GUI as the InventoryView was not created."));
            if(isOpen) close();
            return false;
        }
        ...
        if(settings == null) {
            logger.warn(AdventureUtil.deserialize("Unable to update the suction amount upgrade GUI as the plugin settings are invalid."));
            if(isOpen) close();
            return false;
        }

        TreeMap<Integer, Double> upgrades = settingsManager.getSuctionAmountUpgrades();
        if(upgrades == null) {
            logger.warn(AdventureUtil.deserialize("Unable to update the suction amount upgrade GUI as the suction amount upgrade settings are invalid."));
            if(isOpen) close();
            return false;
        }
    ...
    private void createExitButton() {
        ...
        if(buttonConfig.slot() == null) {
            logger.warn(AdventureUtil.deserialize("Unable to create the exit button in the suction amount upgrade gui due to no slot configured."));
            return;
        }
    ...
    /**
     * Creates the button to increase the suction amount.
     */
    private void createIncreaseButton(@NotNull TreeMap<Integer, Double> upgrades) {
        assert guiConfig != null;

        if(skyHopper.getSuctionAmount() != skyHopper.getMaxSuctionAmount()) {
            ButtonConfig buttonConfig = guiConfig.entries().increase();

            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.deserialize("Unable to create the increase button in the suction amount upgrade gui due to no slot configured."));
                return;
            }

            Optional<Map.Entry<Integer, Double>> optionalUpgrade = getLargerAmount(upgrades, skyHopper.getSuctionAmount());
            ...
            if(optionalItemStack.isPresent()) {
                ...
                buttonBuilder.setAction(inventoryClickEvent -> {
                    skyHopper.setSuctionAmount(upgrade.getKey());
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.deserialize("Unable to create the increase max button in the suction amount upgrade gui due to no slot configured."));
                return;
            }
    ...
    /**
     * Creates the button to decrease the suction amount.
     */
    private void createDecreaseButton(@NotNull TreeMap<Integer, Double> upgrades) {
        assert guiConfig != null;

        if(skyHopper.getSuctionAmount() != upgrades.firstKey()) {
            ButtonConfig buttonConfig = guiConfig.entries().decrease();

            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.deserialize("Unable to create the decrease button in the suction amount upgrade gui due to no slot configured."));
                return;
            }

            Optional<Map.Entry<Integer, Double>> optionalUpgrade = getSmallerAmount(upgrades, skyHopper.getSuctionAmount());
            if(optionalUpgrade.isEmpty()) return;
            Map.Entry<Integer, Double> upgrade = optionalUpgrade.get();
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.deserialize("Unable to create the decrease min button in the suction amount upgrade gui due to no slot configured."));
                return;
            }
    ...
    /**
     * Creates the button to upgrade the suction amount.
     */
    private void createUpgradeButton(@NotNull TreeMap<Integer, Double> upgrades) {
        assert guiConfig != null;

        Optional<Map.Entry<Integer, Double>> nextUpgrade = getNextUpgrade(upgrades, skyHopper.getMaxSuctionAmount());
        if(nextUpgrade.isPresent()) {
            ButtonConfig buttonConfig = guiConfig.entries().upgrade();

            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.deserialize("Unable to create the upgrade button in the suction amount upgrade gui due to no slot configured."));
                return;
            }
            ...
            if(optionalItemStack.isPresent()) {
                ...
                buttonBuilder.setAction(inventoryClickEvent -> {
                    ...
                    if(economyHook.getBalance(player) >= upgradePrice) {
                        ...
                        player.sendMessage(AdventureUtil.deserialize(player, locale.prefix() + locale.suctionAmountUpgrade(), messagePlaceholders));
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.deserialize("Unable to create the upgrade max button in the suction amount upgrade gui due to no slot configured."));
                return;
            }
    ...
    private void createDummyButtons() {
        ...
        guiConfig.entries().dummyButtons().forEach(buttonConfig -> {
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtil.deserialize("Unable to add a dummy button to the suction amount upgrade GUI due to an invalid slot."));
                return;
            }
     ...
     * @param upgrades The TreeMap of upgrades.
     * @param currentKey The current upgrade.
     * @return An Optional of the next suction amount.
     */
     ...
     * @param upgrades The TreeMap of upgrades.
     * @param currentKey The current upgrade.
     * @return An Optional of the previous suction amount.
     */
