 ...
 */
public class TransferSpeedUpgradeGUI extends ChestGUI {
    ...
    public TransferSpeedUpgradeGUI(
        ...
        guiConfig = guiManager.getUpgradeConfig("transfer_speed.yml");
        if(guiConfig == null) {
            throw new RuntimeException("Unable to find loaded config file transfer_speed.yml.");
        }

        guiType = GUIType.getType(guiConfig.guiType());
        if(guiType == null) {
            throw new RuntimeException("Unknown GUIType " + guiConfig.guiType() + " in transfer_speed.yml");
        }

        if(guiConfig.name() == null) {
            throw new RuntimeException("GUI name is null in transfer_speed.yml.");
        }
    ...
    private void createIncreaseButton(@NotNull SkyHopper skyHopper, @NotNull TreeMap<Double, Double> displayUpgrades) {
        ...
        if(skyHopper.transferSpeed() != skyHopper.maxTransferSpeed()) {
            ...
            if (increaseConfig.item().material() != null) {
                ...
                if (material != null) {
                    ...
                    Optional<Map.Entry<Double, Double>> displayUpgrade = getFasterSpeed(displayUpgrades, skyHopper.transferSpeed());
                    if (displayUpgrade.isEmpty()) return;

                    List<TagResolver.Single> lorePlaceholders = List.of(
                            Placeholder.parsed("current", String.valueOf(skyHopper.transferSpeed())),
                    ...
                    increaseBuilder.setAction(inventoryClickEvent -> {
                        ...
                        Optional<Map.Entry<Double, Double>> optionalUpgrade = getFasterSpeed(upgrades, skyHopper.transferSpeed());
    ...
    private void createDecreaseButton(@NotNull SkyHopper skyHopper, @NotNull TreeMap<Double, Double> displayUpgrades) {
        ...
        if(skyHopper.transferSpeed() != displayUpgrades.lastKey()) {
            ...
            if (decreaseConfig.item().material() != null) {
                ...
                if (material != null) {
                    ...
                    Optional<Map.Entry<Double, Double>> displayUpgrade = getSlowerSpeed(displayUpgrades, skyHopper.transferSpeed());
                    if (displayUpgrade.isEmpty()) return;
                    ...
                    decreaseBuilder.setAction(inventoryClickEvent -> {
                        ...
                        Optional<Map.Entry<Double, Double>> optionalUpgrade = getSlowerSpeed(upgrades, currentSkyHopper.transferSpeed());
                        if (optionalUpgrade.isPresent()) {
                            Map.Entry<Double, Double> upgrade = optionalUpgrade.get();
    ...
    private void createUpgradeButton(@NotNull SkyHopper skyHopper, @NotNull TreeMap<Double, Double> displayUpgrades) {
        ...
        Optional<Map.Entry<Double, Double>> displayUpgrade = getNextUpgrade(displayUpgrades, skyHopper.maxTransferSpeed());
        if (displayUpgrade.isPresent()) {
            ...
            if (upgradeConfig.item().material() != null) {
                ...
                if(material != null) {
                    ...
                    List<TagResolver.Single> lorePlaceholders = List.of(
                            Placeholder.parsed("current", String.valueOf(skyHopper.maxTransferSpeed())),
                    ...
                    upgradeBuilder.setAction(inventoryClickEvent -> {
                        ...
                        Optional<Map.Entry<Double, Double>> optionalUpgrade = getNextUpgrade(upgrades, skyHopper.maxTransferSpeed());
                        ...
                        if(plugin.getEconomy().getBalance(player) >= upgradePrice) {
                            if(currentSkyHopper.location() != null && currentSkyHopper.location().getBlock().getState(false) instanceof Hopper hopper) {
                                ...
                                List<TagResolver.Single> placeholders = List.of(
                                        Placeholder.parsed("current", String.valueOf(currentSkyHopper.transferSpeed())),
                                        Placeholder.parsed("next", String.valueOf(upgradeSpeed)));

                                player.sendMessage(FormatUtil.format(player, locale.prefix() + locale.transferSpeedUpgrade(), placeholders));
