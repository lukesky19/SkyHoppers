 ...
 */
public class SelectPlayerGUI extends ChestGUI {
     ...
     * @param membersGUI The MembersGUI the player came from.
    ...
    public SelectPlayerGUI(
        ...
        guiConfig = guiManager.getGuiConfig("select_player.yml");
        if(guiConfig == null) {
            throw new RuntimeException("Unable to find loaded config file select_player.yml.");
        }

        guiType = GUIType.getType(guiConfig.guiType());
        if(guiType == null) {
            throw new RuntimeException("Unknown GUIType " + guiConfig.guiType() + " in select_player.yml.");
        }

        if(guiConfig.name() == null) {
            throw new RuntimeException("GUI name is null in select_player.yml.");
        }
    ...
    public void update() {
        ...
        int playerCount = plugin.getServer().getOnlinePlayers().size() - 1;

        createFiller(guiSize);
        createPlayerButtons(skyHopper, guiSize, playerCount);
    ...
    /**
     * Closes the current GUI and re-opens the MembersGUI.
     * @param plugin The SkyHoppers plugin.
    ...
    public void close(@NotNull Plugin plugin, @NotNull Player player) {
        ...
        membersGUI.open(plugin, player);
    }

    /**
     * Completely closes the GUI and doesn't re-open the MembersGUI.
     * @param plugin The SkyHoppers Plugin.
    ...
    /**
     * Creates the buttons for all Players who can be added to this SkyHopper as a member.
     ...
     */
    private void createPlayerButtons(@NotNull SkyHopper skyHopper, int guiSize, int playerCount) {
        List<Player> onlinePlayers = ImmutableList.copyOf(plugin.getServer().getOnlinePlayers());
        List<UUID> guiMembersList = skyHopper.members();
        GUIConfig.Item itemConfig = guiConfig.entries().playerHead().item();

        if(guiSize - 10 >= 17) {
            for (int i = 0; i <= guiSize - 10; i++) {
                if (playerCount >= playerNum) {
                    UUID uuid = onlinePlayers.get(playerNum).getUniqueId();

                    if(guiMembersList.contains(uuid) || (skyHopper.owner() != null && skyHopper.owner().equals(uuid))) {
                        i--;
                    } else {
