 ...
 */
public class TransferAmountUpgradeGUI extends ChestGUI {
    ...
    public TransferAmountUpgradeGUI(
        ...
        guiConfig = guiManager.getUpgradeConfig("transfer_amount.yml");
        if(guiConfig == null) {
            throw new RuntimeException("Unable to find loaded config file transfer_amount.yml.");
        }

        guiType = GUIType.getType(guiConfig.guiType());
        if(guiType == null) {
            throw new RuntimeException("Unknown GUIType " + guiConfig.guiType() + " in transfer_amount.yml");
        }

        if(guiConfig.name() == null) {
            throw new RuntimeException("GUI name is null in transfer_amount.yml.");
        }
    ...
    public void update() {
        ...
        TreeMap<Integer, Double> displayUpgrades = settingsManager.getTransferAmountUpgrades();
    ...
    private void createIncreaseButton(@NotNull SkyHopper skyHopper, @NotNull TreeMap<Integer, Double> displayUpgrades) {
        ...
        if(skyHopper.transferAmount() != skyHopper.maxTransferAmount()) {
            ...
            if (increaseConfig.item().material() != null) {
                ...
                if (material != null) {
                    ...
                    Optional<Map.Entry<Integer, Double>> displayUpgrade = getLargerAmount(displayUpgrades, skyHopper.transferAmount());
                    if (displayUpgrade.isEmpty()) return;

                    List<TagResolver.Single> lorePlaceholders = List.of(
                            Placeholder.parsed("current", String.valueOf(skyHopper.transferAmount())),
                    ...
                    increaseBuilder.setAction(inventoryClickEvent -> {
                        ...
                        Optional<Map.Entry<Integer, Double>> optionalUpgrade = getLargerAmount(upgrades, skyHopper.transferAmount());
    ...
    private void createDecreaseButton(@NotNull SkyHopper skyHopper, @NotNull TreeMap<Integer, Double> displayUpgrades) {
        ...
        if(skyHopper.transferAmount() != displayUpgrades.firstKey()) {
            ...
            if(decreaseConfig.item().material() != null) {
                ...
                if(material != null) {
                    ...
                    Optional<Map.Entry<Integer, Double>> displayUpgrade = getSmallerAmount(displayUpgrades, skyHopper.transferAmount());
                    if (displayUpgrade.isEmpty()) return;
                    ...
                    decreaseBuilder.setAction(inventoryClickEvent -> {
                        ...
                        Optional<Map.Entry<Integer, Double>> optionalUpgrade = getSmallerAmount(upgrades, skyHopper.transferAmount());
                        if(optionalUpgrade.isPresent()) {
                            Map.Entry<Integer, Double> upgrade = optionalUpgrade.get();
     ...
     */
    private void createUpgradeButton(@NotNull SkyHopper skyHopper, TreeMap<Integer, Double> displayUpgrades) {
        GUIButton.Builder upgradeBuilder = new GUIButton.Builder();

        Optional<Map.Entry<Integer, Double>> displayUpgrade = getNextUpgrade(displayUpgrades, skyHopper.maxTransferAmount());
        if(displayUpgrade.isPresent()) {
            ...
            if (upgradeConfig.item().material() != null) {
                ...
                if(material != null) {
                    ...
                    double displayAmount = displayUpgrade.get().getKey();
                    double displayPrice = displayUpgrade.get().getValue();

                    List<TagResolver.Single> lorePlaceholders = List.of(
                            Placeholder.parsed("current", String.valueOf(skyHopper.maxTransferAmount())),
                            Placeholder.parsed("next", String.valueOf(displayAmount)),
                    ...
                    upgradeBuilder.setAction(inventoryClickEvent -> {
                        ...
                        Optional<Map.Entry<Integer, Double>> optionalUpgrade = getNextUpgrade(upgrades, skyHopper.maxTransferAmount());
                        if(optionalUpgrade.isEmpty()) {
                            player.sendMessage(FormatUtil.format(player, locale.prefix() + locale.upgradeMaxed()));
                            return;
                        }

                        int upgradeAmount = optionalUpgrade.get().getKey();
                        ...
                        if(plugin.getEconomy().getBalance(player) >= upgradePrice) {
                            if(currentSkyHopper.location() != null && currentSkyHopper.location().getBlock().getState(false) instanceof Hopper hopper) {
                                ...
                                List<TagResolver.Single> placeholders = List.of(
                                        Placeholder.parsed("current", String.valueOf(currentSkyHopper.transferAmount())),
                                        Placeholder.parsed("next", String.valueOf(upgradeAmount)));

                                player.sendMessage(FormatUtil.format(player, locale.prefix() + locale.transferAmountUpgrade(), placeholders));
     ...
     */
    private Optional<Map.Entry<Integer, Double>> getLargerAmount(TreeMap<Integer, Double> upgrades, Integer currentKey) {
        Map.Entry<Integer, Double> currentEntry = upgrades.floorEntry(currentKey);
     ...
     */
    private Optional<Map.Entry<Integer, Double>> getSmallerAmount(TreeMap<Integer, Double> upgrades, Integer currentKey) {
        Map.Entry<Integer, Double> currentEntry = upgrades.floorEntry(currentKey);
