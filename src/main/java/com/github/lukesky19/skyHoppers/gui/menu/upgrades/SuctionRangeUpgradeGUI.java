...
public class SuctionRangeUpgradeGUI extends SkyHopperGUI {
    ...
    public boolean create() {
        if(guiConfig == null) {
            logger.warn(AdventureUtility.plain("Unable to create the InventoryView for the suction range upgrade GUI due to invalid GUI configuration."));
            return false;
        }

        GUIType guiType = guiConfig.guiType();
        if(guiType == null) {
            logger.warn(AdventureUtility.plain("Unable to create the InventoryView for the suction range upgrade GUI due to an invalid GUIType"));
            return false;
        }

        String guiName = guiConfig.name();
        if(guiName == null) {
            logger.warn(AdventureUtility.plain("Unable to create the InventoryView for the suction range upgrade GUI due to an invalid gui name."));
            return false;
        }
    ...
    public boolean update() {
        if(guiConfig == null) {
            logger.warn(AdventureUtility.plain("Unable to decorate the GUI due to invalid configuration for the suction range upgrade GUI."));
            if(isOpen) close();
            return false;
        }

        if(inventoryView == null) {
            logger.warn(AdventureUtility.plain("Unable to update the suction range upgrade GUI as the InventoryView was not created."));
            if(isOpen) close();
            return false;
        }
        ...
        if(settings == null) {
            logger.warn(AdventureUtility.plain("Unable to update the suction range upgrade GUI as the plugin settings are invalid."));
            if(isOpen) close();
            return false;
        }
        ...
        if(upgrades == null) {
            logger.warn(AdventureUtility.plain("Unable to update the suction range upgrade GUI as the suction range upgrade settings are invalid."));
            if(isOpen) close();
            return false;
        }
    ...
    private void createExitButton() {
        ...
        if(buttonConfig.slot() == null) {
            logger.warn(AdventureUtility.plain("Unable to create the exit button in the suction range upgrade gui due to no slot configured."));
            return;
        }
    ...
    private void createIncreaseButton(@NotNull TreeMap<Integer, Double> upgrades) {
        ...
        if(skyHopper.getSuctionRange() != skyHopper.getMaxSuctionRange()) {
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtility.plain("Unable to create the increase button in the suction range upgrade gui due to no slot configured."));
                return;
            }
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtility.plain("Unable to create the increase max button in the suction range upgrade gui due to no slot configured."));
                return;
            }
    ...
    private void createDecreaseButton(@NotNull TreeMap<Integer, Double> upgrades) {
        ...
        if(skyHopper.getSuctionRange() != upgrades.firstKey()) {
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtility.plain("Unable to create the decrease button in the suction range upgrade gui due to no slot configured."));
                return;
            }
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtility.plain("Unable to create the decrease min button in the suction range upgrade gui due to no slot configured."));
                return;
            }
    ...
    private void createUpgradeButton(@NotNull TreeMap<Integer, Double> upgrades) {
        ...
        if(nextUpgrade.isPresent()) {
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtility.plain("Unable to create the upgrade button in the suction range upgrade gui due to no slot configured."));
                return;
            }
            ...
            if(optionalItemStack.isPresent()) {
                ...
                buttonBuilder.setAction(_ -> {
                    ...
                    if(economyHook.getBalance(player) >= upgradePrice) {
                        ...
                        player.sendMessage(PaperAdventureUtility.deserialize(player, locale.prefix() + locale.suctionRangeUpgrade(), messagePlaceholders));
            ...
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtility.plain("Unable to create the upgrade max button in the suction range upgrade gui due to no slot configured."));
                return;
            }
    ...
    private void createDummyButtons() {
        ...
        guiConfig.entries().dummyButtons().forEach(buttonConfig -> {
            if(buttonConfig.slot() == null) {
                logger.warn(AdventureUtility.plain("Unable to add a dummy button to the suction range upgrade GUI due to an invalid slot."));
                return;
            }
